...
export function useAuth() {
  ...
  // Load user from localStorage on initialization - but only if this is not a new deployment
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      const fetchAllUsers = async () => {
        try {
          ...
          if (response.ok) {
            ...
            // Sync cache with server data (this replaces all cached users)
      ...
      try {
        // Check if this is the first time the site has been loaded (new deployment)
        const isFirstVisit = !localStorage.getItem('site_visited');
        
        // If this is the first visit (new deployment), mark it but don't prevent authentication
        if (isFirstVisit) {
          ...
          // Don't clear user data or prevent auto-login - let normal auth flow continue
        }
        ...
        try {
          ...
          if (sessionResponse.ok) {
            ...
            // Handle date string conversion back to Date object
      ...
      // Fallback: Normal login process for returning visitors using localStorage
      ...
      if (storedUser) {
        ...
        if (parsedUser && parsedUser.username) {
          ...
          // For DigQuestor, fetch fresh user data from database to get updated avatar
          if (parsedUser.username === "DigQuestor") {
            ...
            // Debug: Check if the avatar URL is present in the stored user data
            if (parsedUser.avatarUrl) {
              // Truncate for logging - may be a long base64 string
